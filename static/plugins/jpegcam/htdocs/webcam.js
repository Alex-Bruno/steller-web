...
window.webcam = {
	...
	swf_url: '/static/plugins/jpegcam/htdocs/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url: '/static/plugins/jpegcam/htdocs/shutter.mp3', // URI to shutter.mp3 sound
	api_url: '', // URL to upload script
	loaded: false, // true when webcam movie finishes loading
	quality: 90, // JPEG quality (1 - 100)
	shutter_sound: true, // shutter sound effect on/off
	stealth: false, // stealth mode (do not freeze images upon capture)
	hooks: {
		onLoad: null,
		onComplete: null,
		onError: null
	...
	freeze: function() {
		// freeze webcam images (capture but do not upload)
		this.get_movie()._snap('', this.quality, this.shutter_sound ? 1 : 0, 0 );
	},
	
	upload: function(url, callback) {
		// upload images to server after taking snapshot
		// specify fully-qualified URL to server API script
		// and callback function (string or function object)
		if (callback) this.set_hook('onComplete', callback);
		if (url) this.set_api_url(url);
	...
	set_shutter_sound: function(enabled, url) {
		...
		this.shutter_url = url ? url : '/static/plugins/jpegcam/htdocs/shutter.mp3';
	},
